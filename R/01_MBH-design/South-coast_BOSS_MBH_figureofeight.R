...
tifs.fig  <- list.files("data/spatial/rasters/", "figureeight*", full.names = TRUE)
preds.fig <- stack(tifs.fig)
poly <- vect("data/mbh-design/Figure8_sample-polys.shp")
ext(poly)
preds.fig <- crop(preds.fig, extent(121.580592920142, 121.665297653349, -34.0685064016901, -33.9973183812295))
plot(preds.fig)

# Make cuts for Figure of Eight
n <- 30                                                                         # Number of samples

summary(preds.fig)
hist(preds.fig$figureeight_roughness)
roughness_qs <- c(0, 0.7, 0.9, 1)
roughness_cuts   <- quantile(preds.fig$figureeight_roughness, probs = roughness_qs)
cat_roughness  <- cut(preds.fig$figureeight_roughness, breaks = roughness_cuts, na.rm = TRUE)
plot(stack(preds.fig$figureeight_roughness, cat_roughness))
...
samp <- samp %>%
  dplyr::mutate(sample = paste("FIG-DC", 
                               str_pad(row_number(), 2,                     
                                       side = "left", pad = "0") , sep = "-"))

## Write out sampling file 
write.csv(samp,"data/mbh-design/Figureofeight_MBH_wgs84.csv") # write out the each region
