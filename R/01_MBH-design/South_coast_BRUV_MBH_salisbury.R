...
tifs.sal  <- list.files("data/spatial/rasters/", "salisbury*", full.names = TRUE)
preds.sal <- stack(tifs.sal)
extent(preds.sal)
# preds.sal <- crop(preds.sal, extent(120.855, 121.18, -34.15, -33.895))
plot(preds.sal)

# Make cuts for Salisbury
n <- 72                                                                         # Number of samples

hist(preds.sal$salisbury_roughness)
roughness_qs <- c(0, 0.7, 0.9, 1)
roughness_cuts   <- quantile(preds.sal$salisbury_roughness, probs = roughness_qs)
cat_roughness  <- cut(preds.sal$salisbury_roughness, breaks = roughness_cuts, na.rm = TRUE)
plot(stack(preds.sal$salisbury_roughness, cat_roughness))
...
samp <- samp %>%
  dplyr::mutate(sample = paste("SAL-BV", 
                               str_pad(row_number(), 2,                     
                                       side = "left", pad = "0") , sep = "-"))

## Write out sampling file 
write.csv(samp,"data/mbh-design/Salisbury_BRUVs_MBH_wgs84.csv") # write out the each region
