...
# BOSS regular spec
boss.cameras <- read.csv("data/metadata/2023-06_Nat-reef_second-trip/2023-06_Naturaliste-Reefs-2_BOSS_cameras.csv") %>%
  ga.clean.names() %>%
...
boss.metadata <- read.csv("data/metadata/2023-06_Nat-reef_second-trip/2023-06_Naturaliste-Reefs-2_BOSS_metadata.csv") %>%
  ga.clean.names() %>%
  dplyr::filter(!sample %in% c("ST08", "St11", "ST07", "ST12")) %>%             # Remove soundtrap samples
...
# BOSS soundtrap spec
boss.cameras <- read.csv("data/metadata/2023-06_Nat-reef_second-trip/2023-06_Naturaliste-Reefs-2_Soundtrap-BOSS_cameras.csv") %>%
  ga.clean.names() %>%
  dplyr::rename(system.number = system.)%>%
  dplyr::mutate(time = mdy_hms(creationdate, tz = "UTC"))  %>%
  dplyr::mutate(local.time = with_tz(time, tz = "Australia/Perth")) %>%
...
# this is the only one that goes to the metadata!!
boss.metadata <- read.csv("data/metadata/2023-06_Nat-reef_second-trip/2023-06_Naturaliste-Reefs-2_Soundtrap-BOSS_metadata.csv") %>%
  ga.clean.names() %>%
  dplyr::rename(system.number = system., depth = depth.m., longitude = x, latitude = y, video.notes = notes) %>%
  dplyr::mutate(depth = conv_unit(depth, "fathom", "m")) %>%
  dplyr::mutate(time = mdy_hms(creationdate, tz = "UTC")) %>%
...
missing.samps <- read.csv("data/metadata/2023-06_Nat-reef_second-trip/2023-06_Naturaliste-Reefs-2_BOSS_metadata.csv") %>%
  ga.clean.names() %>%
  dplyr::filter(sample %in% c("ST08", "St11", "ST07", "ST12")) %>%             # Remove regular BOSS samples
  dplyr::rename(system.number = system., depth = depth.m., longitude = x, latitude = y, video.notes = notes) %>%
  dplyr::mutate(depth = conv_unit(depth, "fathom", "m")) %>%
  dplyr::mutate(time = mdy_hms(creationdate, tz = "UTC")) %>%
  dplyr::mutate(local.time = with_tz(time, tz = "Australia/Perth")) %>%
...
boss.metadata <- bind_rows(boss.metadata, missing.samps) %>%
  arrange(time) %>%
  glimpse()
